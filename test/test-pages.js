...
describe("Url Check", () => {
  ...
  it("RegisterPage Status", function (done) {
    this.timeout(15000);
    server.get("/register").end((err, res) => {
      expect(res.status).to.equal(200);
      if (err) done(err);
      done();
    });
  ...
  it("Direct ProductPage Status", function (done) {
    this.timeout(15000);
    server.get("/product").end((err, res) => {
  ...
  it("Get WishList", function (done) {
    this.timeout(15000);
    server.get("/wishlist").end((err, res) => {
      expect(res.status).to.equal(200);
      if (err) done(err);
      done();
    });
  ...
  it("Get Checkout", function (done) {
    this.timeout(15000);
    server.get("/checkout").end((err, res) => {
      expect(res.status).to.equal(200);
      if (err) done(err);
      done();
    });
