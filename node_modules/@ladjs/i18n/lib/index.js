...
class I18N {
  constructor(config = {}) {
    ...
    if (!every(this.config.locales, l => locales.includes(l))) throw new Error(`Invalid locales: ${this.config.locales.filter(str => !locales.includes(str)).join(', ')}`); // default locale must be in locales

    if (!this.config.locales.includes(this.config.defaultLocale)) throw new Error(`Default locale of ${this.config.defaultLocale} must be included in list of locales`); // inherit i18n object
  ...
  middleware(ctx, next) {
    ...
    if (!locale) {
      locale = defaultLocale; // if "Accepts: */*" or "Accept-Language: */*"
      ...
      const acceptedLocale = ctx.request.acceptsLanguages([...new Set([defaultLocale, ...locales])]);
